...
class BluetoothService(private val handler: Handler, private val callback: IBluetoothServiceListener) {
    ...
    @Synchronized
    private fun onClientConnectionSucceded(bluetoothSocket: BluetoothSocket) {
        connectedThread?.cancel()
        connectedThread = null
    ...
    private inner class ClientConnectionThread(device: BluetoothDevice, uuid: UUID,
        ...
        init {
            ...
            try {
                ...
                // uuid is the app's UUID buffer, also used in the server daysCode.
        ...
        override fun run() {
            ...
            try {
                ...
                if (bluetoothSocket?.isConnected == true)
                    onClientConnectionSucceded(bluetoothSocket)
    ...
    private class ConnectedThread(private val bluetoothSocket: BluetoothSocket,
                ...
                try {
                    ...
                    if (inbox == '\n'.toInt()) {
                        handler.obtainMessage(
                                ...
                                -1,
                                Bundle().apply {
                                    putParcelable(BUNDLE_KEY_DEVICE,
                                            bluetoothSocket.remoteDevice)
                                    putString(BUNDLE_KEY_MESSAGE, buffer)
                                }
                        ).sendToTarget()
                        buffer = ""
        ...
        fun write(data: String) {
            try {
                ...
                //android.util.Log.e(javaClass.name, "Enviado: $data")
